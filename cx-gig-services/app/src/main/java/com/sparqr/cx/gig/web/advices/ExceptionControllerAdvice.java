...
import com.sparqr.cx.gig.services.exceptions.MissingRequiredFieldsException;
...
public class ExceptionControllerAdvice implements ProblemHandling {
  ...
  /**
   * Handles GigNotActiveException in controller.
   *
   * @param exception {@link GigNotActiveException}
   * @return Problem GIG-002
   */
  @ExceptionHandler(GigNotActiveException.class)
  public ResponseEntity<Problem> handleException(
      GigNotActiveException exception) {

    log.error("Error: {}", exception.getMessage());

    return ResponseEntity.status(BAD_REQUEST.getStatusCode())
        .contentType(MediaTypes.PROBLEM)
        .body(Problems.gigNotActive(exception.getId()));
  }

  /**
   * Handles GigNotFoundException in controller.
   *
   * @param exception {@link GigNotFoundException}
   * @return Problem GIG-001
   */
  @ExceptionHandler(GigNotFoundException.class)
  public ResponseEntity<Problem> handleException(
      GigNotFoundException exception) {

    log.error("Error: {}", exception.getMessage());

    return ResponseEntity.status(BAD_REQUEST.getStatusCode())
        .contentType(MediaTypes.PROBLEM)
        .body(Problems.gigNotFound(exception.getId()));
  }
