...
import com.sparqr.cx.iam.services.exceptions.MissingRequiredFieldsException;
...
public class ExceptionControllerAdvice implements ProblemHandling {
  ...
  /**
   * Handles UserNotFoundException in controller.
   *
   * @param exception {@link UserNotFoundException}
   * @return Problem IAM-001 and IAM-002
   */
  ...
  public ResponseEntity<Problem> handleException(
    ...
    if (StringUtils.hasLength(exception.getAlias())) {
      return ResponseEntity.status(BAD_REQUEST.getStatusCode())
          .contentType(MediaTypes.PROBLEM)
          .body(Problems.userNotFound(exception.getAlias()));

    } else {
      return ResponseEntity.status(BAD_REQUEST.getStatusCode())
          .contentType(MediaTypes.PROBLEM)
          .body(Problems.userNotFound(exception.getId()));
    }
  }
